...
import com.tjerkw.slideexpandable.library.RotateAnimation.InterpolatedTimeListener;
 ...
 * @date 6/9/12 4:41 PM
 */
public abstract class AbstractFlipListAdapter extends WrapperListAdapterImpl{
	/**
	...
	public AbstractFlipListAdapter(ListAdapter wrapped) {
		super(wrapped);
	}
	 ...
	 */
	public abstract View getFlipFrontView(View parent);
	 ...
	 */
	public abstract View getFlipBehindView(View parent);
	 ...
	 */
	public boolean isAnyItemFlip() {
		return (lastOpenPosition != -1) ? true : false;
	}



	private void enableFor(final View target,final int position) {
		...
		View frontView = getFlipFrontView(target);
		View behindView = getFlipBehindView(target);
		if(frontView != null){
			frontView.setOnClickListener(new View.OnClickListener() {
				@Override
				public void onClick(View view) {
					if(enableFlip){
						open(target, view, position,RotateAnimation.ROTATE_DECREASE);
					}
				}
			});
		}
		if(behindView != null){
			behindView.setOnClickListener(new View.OnClickListener() {
				@Override
				public void onClick(View view) {
					if(enableFlip){
						open(target, view, position,RotateAnimation.ROTATE_INCREASE);
					}
				}
			});
		}
	...
	private void open(View target,final View view,final int position,int animType){
		...
		if (a != null && a.hasStarted() && !a.hasEnded()) {
			...
			// remember the state
			if (animType == RotateAnimation.ROTATE_DECREASE) {
			...
			if (animType == RotateAnimation.ROTATE_DECREASE) {
				if (lastOpenPosition != -1 && lastOpenPosition != position) {
					if (lastOpen != null) {
						animateView(lastOpen, RotateAnimation.ROTATE_INCREASE);
					}
	...
	private void animateView(final View target, final int type) {
		RotateAnimation anim = new RotateAnimation(target.getWidth()/2
				,target.getHeight()/2
				,type
		);
	...
	public void setHint(View parent,int type) {
		getFlipFrontView(parent).setVisibility(View.GONE);
		getFlipBehindView(parent).setVisibility(View.GONE);
		if(type == RotateAnimation.ROTATE_DECREASE){
			getFlipBehindView(parent).setVisibility(View.VISIBLE);
		}else{
			getFlipFrontView(parent).setVisibility(View.VISIBLE);
		}
	}
